...
# library("mnlogit")
# library("RSGHB")
# fm <- formula(Choice ~ price | income | catch)
...
# summary(Mn)
# Mrs <- 
# Pmn <- predict(Mn, newdata = S)
# PredMix <- 0.9*Pmn+0.1*pred
...
train <- read.table("trainnormall_1.csv", sep = ",", header = TRUE)
...
modelname <- "MNL_lognormal" # used for output

# Names for the random variables
gVarNamesNormal <- c("Bx", "By")

# Fixed Variables
gVarNamesFixed <- c() # None, can be omitted
...
plot(model)
plot(model, type = "A") # Note: these are the chains corresponding to the mean of the normals underlying the log-normal distributions
