...
summary(train1$year)
# a <- subset(train, train$Urb=="Urban/City")
# table(a$Price1)
# trainnorm <- read.csv("trainnormall.csv")
# train$oldeduc <- train$old*train$educa
# which(colnames(train)=="oldeduc")
# for (i in 1:nrow(train)){
#   train[i,111] <- ((train[i,111]-min(train[,111]))/(max(train[,111])-min(train[,111])))
# }
# train$oldeduc1 <- as.numeric(train$Price1)*train$year
# train$oldeduc2<- as.numeric(train$Price2)*train$year
# train$year3 <- as.numeric(train$Price3)*train$year
# train$year4 <- as.numeric(train$Price4)*train$year
#trainnorm$oldeduc <-train$oldeduc
str(train)
summary(train)
...
# 
# for (i in 1:nrow(train)){
# if (train$Choice[i]=="Ch1") {
#    train$cate[i]<-1
#  } else if ( train$Choice[i]=="Ch2") {
#    train$cate[i]<-2
#  } else if ( train$Choice[i]=="Ch3") {
#    train$cate[i]<-3
#  }else {
#     train$cate[i]<-4
#  }}
...
# summary(newtrain)
write.csv(train,"trainnormall_2.csv")
# nottoonew <- train[c(1:63,84:94,109)]
# summary(nottoonew)
